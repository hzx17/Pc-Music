<!-- html -->
<template>
  <div v-if="videoOptions.src" class='mv-container'>
    <div class="mv-main">
      <!-- mv标题 -->
      <div class="mv-info-hd">
        <div class="info-name"><i class="iconfont icon-mv"></i>{{mvDetail.name}}</div>
        <div class="song-author" v-for="(author, k) in mvDetail.artists" :key="author.name">{{ k !== 0 ? ' / ' + author.name : author.name }}</div>
      </div>
      <div class="mv-inner">
      ...
      <div class="mv-comment">评论组件，开发中</div>
    </div>
    <div class="mv-aside">
        <div class="mv-info">
          ...
          <div class="aside-main">
              <div class="item" :key="'' + item.id + index" v-for="(item, index) in simiMv">
                  ...
                  <div class="info">
                      ...
                      <div class="mv-author" v-if="!item.publishTime">{{item.artistName}}</div>
...
import { getCurrentInstance,toRefs , onMounted, reactive } from '@vue/runtime-core'
